...
public class MaterialCreateWindow : EditorWindow
{
    ...
    private void OnEnable()
    {
        ...
        //if()
        var findAsset = AssetDatabase.FindAssets("EditorToolTextureMap")
            .Select(guid => AssetDatabase.GUIDToAssetPath(guid))
            .Select(path => AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(path));
        // TextureMapSO findMapSO = default;
        if(findAsset.Count() != 0)
        {
            textureMapSO = findAsset.First() as TextureMapSO;
        }
        ...
        if (bIsFrom3dTexture)
        {
            ...
            foreach (var item in dirInfo.GetFiles())
            {
                ...
                if (true) // temp statement
                {
                    ...
                    var tempIndex = 0;
                    if (textureTypeDic.Count == 0)
                    {
                        foreach (var stringItem in stringPatterns)
                        {
                            Debug.Log("temp index " + tempIndex
                                + " string item " + stringItem);
                            textureTypeDic.Add(stringItem, (eCurrentTexture)tempIndex);
                            tempIndex++;
                        }
                    }
                    var currentTextureType = GetCurrentTextureType(assetPath, compareStringData, textureTypeDic);
